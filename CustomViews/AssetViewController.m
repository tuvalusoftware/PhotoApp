...
- (NSData *)_jpegRepresentationWithCompressionQuality:(CGFloat)compressionQuality {
   ...
   uiImage = [uiImage  resizedImageToSize:CGSizeMake(size.width/2,size.height/2)];
	if (outputImageRef) {
...
-(void) applyResize
{
    ...
    [self.asset requestContentEditingInputWithOptions:options completionHandler:^(PHContentEditingInput *contentEditingInput, NSDictionary *info) {
     ...
     //   ImageProcessing * imageProcessing = [[ImageProcessing alloc] init];
        ...
        [f setValue:[NSNumber numberWithFloat:2.5]
        ...
        CIImage *outputImage  = [f valueForKey:kCIOutputImageKey];

        // Create editing output
        NSData *jpegData = [outputImage _jpegRepresentationWithCompressionQuality:0.9f];
...
- (IBAction)handleEditButtonItem:(id)sender
{
    ...
    if ([self.asset canPerformEditOperation:PHAssetEditOperationContent]) {
        if (self.asset.mediaType == PHAssetMediaTypeImage) {
            ...
            [alertController addAction:[UIAlertAction actionWithTitle:NSLocalizedString(@"Resize", @"") style:UIAlertActionStyleDefault handler:^(UIAlertAction *action) {
                [self applyResize];
            }]];
        }
